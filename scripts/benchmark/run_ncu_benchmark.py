...
def parse_and_calculate(output_text, kernel_id):
    ...
    header = (
        ...
        if header_match
        else f"Running kernel {kernel_id}: Unknown on cuda:0."
    )
    ...
    writes = re.findall(
        r"dram__bytes_write\.sum\s+([a-zA-Z]+)\s+([\d\.,]+)", output_text
    )
    times = re.findall(
        r"gpu__time_duration\.sum\s+([a-zA-Z]+)\s+([\d\.,]+)", output_text
    )
    ...
    for i in range(count):
        ...
        # 格式化输出 (保持和需求一样对齐)
        line = (
            f"{shape} | Time: {time_us_val:>7.2f} µs | Bandwidth: {bw_gbps:>6.1f} GB/s"
        )
        lines.append(line)
...
def main():
    ...
    for kernel_id in range(8):
        ...
        out_file = f"benchmark_results/{kernel_id}_results_kernel.txt"
